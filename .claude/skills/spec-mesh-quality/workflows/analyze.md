...
> **Severity Classification:** constitution/quality-gates.md の Severity Classifications を参照。
...
|-----------|----------|-------------|
| PASS の場合 | pr | PR 作成 |
| WARN の場合 | pr | Issue 確認後 PR 作成 |
| FAIL (未実装) の場合 | analyze | 実装追加後に再分析 |
| FAIL (Spec外実装) の場合 | analyze | Spec 追加 or コード削除後に再分析 |

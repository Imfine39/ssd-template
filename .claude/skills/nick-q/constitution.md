...
```bash
# Tasks 作成時（0/5 タスク完了）
node .claude/skills/nick-q/scripts/state.cjs branch --set-step tasks --set-task-progress 0/5

# タスク完了ごとに更新
node .claude/skills/nick-q/scripts/state.cjs branch --set-task-progress 1/5
node .claude/skills/nick-q/scripts/state.cjs branch --set-task-progress 2/5

# 全タスク完了時
node .claude/skills/nick-q/scripts/state.cjs branch --set-step implement --set-task-progress 5/5
```

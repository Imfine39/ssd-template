 ...
 *   - "Matrix file not found" - Run /nick-q design or create matrix manually
 ...
 *
 * Usage:
 *   node .claude/skills/nick-q/scripts/validate-matrix.cjs [options]
 *
 * Options:
...
Usage: node .claude/skills/nick-q/scripts/validate-matrix.cjs [options]
...
Examples:
  ...
  node .claude/skills/nick-q/scripts/validate-matrix.cjs --fix > suggestions.json
        `);
        process.exit(0);
      }
    }
...
function validate(options) {
  ...
  if (!matrix) {
    ...
    console.log('Run AI-based Matrix generation during /nick-q design or create manually.');
    process.exit(1);
  }

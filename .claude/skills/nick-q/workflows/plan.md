...
# または state.cjs から
node .claude/skills/nick-q/scripts/state.cjs query --branch
```
...
3. **Read Constitution:**
   ```
   Read tool: constitution/core.md (in skill directory)
   ```
...
```bash
node .claude/skills/nick-q/scripts/state.cjs branch --set-step plan
```

...
| # | 観点 | 説明 | 例 |
|---|------|------|-----|
| 1 | 機能追加 | 不足している機能 | 検索機能、フィルタリング |
| 2 | 機能削除 | MVP で不要な機能 | 過剰な設定画面 |
| 3 | データ追加 | 不足しているデータ項目 | 作成日時、更新履歴 |
| 4 | データ削除 | 不要なデータ項目 | 冗長なフィールド |
| 5 | ビジネスロジック | 計算・判定ロジック | 自動計算、ステータス遷移 |
| 6 | UX 改善 | ユーザー体験向上 | 確認ダイアログ、プログレス表示 |
| 7 | セキュリティ | 権限・認証・保護 | 権限管理、入力サニタイズ |
| 8 | パフォーマンス | 速度・効率 | ページネーション、キャッシュ |
| 9 | 保守性 | ログ・監視・運用 | エラーログ、操作履歴 |
| 10 | スケーラビリティ | 将来の拡張 | 多言語対応、マルチテナント |

**提案生成ルール:**
```
1. Input を分析し、上記 10 観点でギャップを特定

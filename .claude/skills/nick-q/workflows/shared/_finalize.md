...
node .claude/skills/nick-q/scripts/reset-input.cjs add

# Fix の場合
node .claude/skills/nick-q/scripts/preserve-input.cjs fix --fix {fix-dir}
node .claude/skills/nick-q/scripts/reset-input.cjs fix
```
...
```bash
# [FUTURE] このコマンドは未実装
# node .claude/skills/nick-q/scripts/state.cjs branch --add-checkpoint spec_approval
```

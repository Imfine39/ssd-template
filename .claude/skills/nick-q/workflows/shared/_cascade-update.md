...
> **SSOT:** [judgment-criteria.md#複合変更の判定基準](../../guides/judgment-criteria.md#複合変更の判定基準) 参照

Add/Fix 実行中に既存 Spec の変更が必要になった場合、以下の基準で分離判定を行う。

### 規模別判定表

| 規模 | 条件 | 対応 | Issue |
|------|------|------|-------|
| **小規模** | 属性 1-3 個追加、単一 Spec 内、新規 ID 不要 | 同一 Issue で対応 | 分離不要 |
| **中規模** | 属性 4-10 個、または 2 Spec に影響 | 分離を提案（ユーザー判断） | 分離推奨 |
| **大規模** | 新規 Master 追加、API 変更、3+ Spec に影響 | 必ず分離 | 分離必須 |

### 判定フロー

```
Add/Fix 実行中に既存 Spec 変更が必要
    │
    ├─ 属性 1-3 個、1 Spec のみ → 小規模 → このまま Cascade Update
    │
    ├─ 属性 4-10 個、または 2 Spec → 中規模
    │   └─ AskUserQuestion:
    │        「この変更は別 Issue に分離しますか？」
    │        - [推奨] 分離する（変更履歴を明確に）
    │        - 同一 Issue で対応する
    │
    └─ 新規 Master/API、3+ Spec → 大規模 → 必ず分離
         └─ change ワークフローを先行実行
```
...
1. 現在のワークフローを一時停止
2. change ワークフローで先行変更を実施
3. 先行変更のマージ後、元のワークフローを再開
...
```bash
# 更新記録の追加
node .claude/skills/nick-q/scripts/state.cjs repo \
...
# 記録の確認
node .claude/skills/nick-q/scripts/state.cjs repo --list-cascade
```
